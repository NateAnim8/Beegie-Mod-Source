...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var mural:FlxSprite = new FlxSprite(0,56).loadGraphic(Paths.image("Beegie Story Mode Mural"));
	...
	function selectWeek()
	{
		if (curWeek >= weekUnlocked.length || weekUnlocked[curWeek])
		{
			...
			//new FlxTimer().start(2, cutscene, 1);
		}
	}

	//var video:MP4Handler = new MP4Handler();
	
	/*function cutscene(timer:FlxTimer):Void
	{
		if (curWeek == 0 && !isCutscene) // Checks if the current week is Tutorial.
			{
				video.playMP4(Paths.video('First_Cutscene_FINISHED'), new PlayState()); 
				isCutscene = true;
			}
			else
			{
