...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'swingin':
				dialogue = CoolUtil.coolTextFile(Paths.txt('swingin/swinginDialogue'));
			case 'popsicle sticks':
				dialogue = CoolUtil.coolTextFile(Paths.txt('popsicle-sticks/popsicle-sticksDialogue'));
			case 'hop-2-it':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hop-2-it/hop-2-itDialogue'));
		}
		switch (SONG.song.toLowerCase())
		{
			...
			case 'teehee' | 'swingin' | 'popsicle sticks':
				curStage = 'beegie';
				defaultCamZoom = 0.8;
				{
						var bg:BGSprite = new BGSprite('BeegieStuff/BeegieBG', -380, -400, 0.9, 0.9);
						add(bg);
				}
			
			case "hop-2-it":
				curStage = 'beegie-eugn';
				defaultCamZoom = 0.7;
				{
						var bg:BGSprite = new BGSprite('BeegieStuff/BeegieBG-EUGN', -380, -400, 0.9, 0.9);
						add(bg);

						bottomBoppers = new BGSprite('BeegieStuff/backgroundbitches', -280, -250, 0.9, 0.9, ['BOPPE']);
						bottomBoppers.animation.addByPrefix('BOPPE', 'BOPPE', 24, false);
						add(bottomBoppers);
				}
			default:
				defaultCamZoom = 0.9;
				...
				//don't put any cases after default
		...
		if (isStoryMode && !seenCutscene || curSong.toLowerCase() == 'hop-2-it')
		{
			switch (daSong)
			{
				case "monster":
				...
				case "teehee":
					...
					video.x -= 320;
					video.y -= 180;
					video.playMP4(Paths.video('First_Cutscene_FINISHED'), function(){
						camHUD.fade(FlxColor.BLACK, 1, true);
						startCountdown();
						remove(video);
					}, false, true);
				...
				case 'popsicle sticks':
					...
					video.x -= 320;
					video.y -= 180;
					video.playMP4(Paths.video('Beegie_Second_Cutscene_FINISHED'), function(){
						camHUD.fade(FlxColor.BLACK, 1, true);
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch(curStage) {
					...
					case 'beegie-eugn'://ya forgot this stage lol
						bottomBoppers.dance(true);
				}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, ?onLua:Bool = false) {
		switch(eventName) {
			case 'Hey!':
				...
				case 'Beegie Teehee':
					if(dad.curCharacter == 'beegie') {
						dad.playAnim('teehee', true);
						dad.specialAnim = true;
				}
	...
	function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:Int = checkForAchievement();
			if(achieve > -1) {
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				switch(curSong.toLowerCase()){
					case "popsicle sticks":
						MusicBeatState.switchState(new SoloCutscene(Paths.video("Beegie Final Cutscene FINISHED"), new MainMenuState()));
					default:
				...
				if(storyWeek < 1) //First week
				{
					FlxG.save.data.hop2itUnlocked = true;
				}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = coolText.x + (43 * daLoop) - 300;
			numScore.y += 100;
			...
			numScore.velocity.x = FlxG.random.float(-10, 5);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.noteType == 2 && boyfriend.animOffsets.exists('hey')) {
				boyfriend.playAnim('hey', true);
				boyfriend.specialAnim = true;
			...
			else if(note.noteType == 4 && boyfriend.animOffsets.exists('teehee')) { //teehee
				boyfriend.playAnim('teehee', true);
				boyfriend.specialAnim = true;
			}
			else {
				var daAlt = '';
	...
	#if ACHIEVEMENTS_ALLOWED
	private function checkForAchievement():Int {
		for (i in 1...9) {
			if(!Achievements.achievementsUnlocked[i][1]) {
				switch(i) {
					case 1:
						if(isStoryMode && campaignMisses + songMisses < 1 && storyDifficulty == 2 &&
						storyPlaylist.length <= 1 && (storyWeek + 1) == i && !changedDifficulty && !usedPractice) {
							Achievements.unlockAchievement(i);
							return i;
						}
					case 2:
						if(songMisses < 1 && storyDifficulty == 2 && PlayState.SONG.song.toLowerCase() == 'hop-2-it' && !changedDifficulty && !usedPractice) {
							Achievements.unlockAchievement(i);
							return i;
						}
					case 3:
